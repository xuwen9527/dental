  ...
  static bool readPTX( OpenMeshType &m, FILE *fp, Info importparams, CallBackPos *cb=NULL)
  {
    ...
    if(! importparams.pointsonly)
    {
      ...
      for(int rit=0; rit<rownum-1; rit++)
        ...
        {
          t++;
          if(cb) cb(50 + (t*50)/(rownum*colnum),"PTX Node Loading");
    ...
    {
      if(cb) cb(40,"PTX Node Loading - remove invalid vertices");
      ...
      if(! importparams.pointsonly)
      {
        if(cb) cb(60,"PTX Node Loading - remove invalid faces");
    ...
    { // Compute Normals and radius for points
      ...
      if(cb) cb(85,"PTX Node Loading - computing vert normals");
      ...
      // eliminate high angle triangles
    {
      ...
      {
        if(cb) cb(85,"PTX Node Loading - remove steep faces");
    ...
    if(cb) cb(100,"PTX Node Loading finished!");
    return true;
  }
