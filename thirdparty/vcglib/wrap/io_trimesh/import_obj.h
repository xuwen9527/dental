        ...
        namespace io {
            ...
            namespace
            {
                std::string getFileDir(const std::string& fileName)
                {
                    ...
                    else return std::string(fileName, 0, slash) + PATH_SEPARATORS;
                }
            }
                ...
                static int Open(OpenMeshType &m, const char * filename, Info &oi)
                {
                    ...
                    while (!stream.eof())
                    {
                        ...
                        if (numTokens > 0)
                        {
                            ...
                            {
                                std::string path = getFileDir(filename);
                                std::string materialFileName = path + tokens[1];
                    ...
                    //-------------------------------------------------------------------------------
                    //convert indexes into pointers for face to vert/norm/tex references
                    //xuwen 20170525
                    if (((oi.mask & vcg::tri::io::Mask::IOM_VERTNORMAL) != 0))
                    {
                        for (int i = 0; i < numVNormals; ++i)
                        {
                            assert(m.vert.size() == size_t(m.vn));
                            m.vert[i].N()[0] = normals[i][0];
                            m.vert[i].N()[1] = normals[i][1];
                            m.vert[i].N()[2] = normals[i][2];
                        }
                    }
...
//							  qDebug("XXXXXX %i",indexedFaces[i].n[j]);
                ...
                * ����һ������������������
                */
                inline static size_t NumTokenizeLine(std::string& line)
                {
                    size_t from = 0;
                    size_t to = 0;
                    size_t length = line.size();

                    size_t tokenize_num = 0;
                    do
                    {
