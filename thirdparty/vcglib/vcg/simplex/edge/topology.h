...
void VEEdgeCollapse(MeshType &poly, typename MeshType::EdgeType *e0, const int z)
{
  ...
  std::allocator<MeshType>::DeleteEdge(poly,*e1);
  std::allocator<MeshType>::DeleteVertex(poly,*vd);
}
...
//  tri::Allocator<MeshType>:: template PointerUpdater<typename MeshType::EdgePointer> pu;
  typename MeshType::EdgeIterator ei = std::allocator<MeshType>::AddEdges(poly, 1);
  ei->V(0)=&v;
  ei->V(1)=v1;
  edge::VEAppend(&*ei,0);
  edge::VEAppend(&*ei,1);
...
{
  typename MeshType::VertexIterator vi = std::allocator<MeshType>::AddVertex(poly,p);
...
{
  typename MeshType::VertexIterator vi = std::allocator<MeshType>::AddVertex(poly,p,n);
  VEEdgeSplit(poly,e,*vi);
  return &*vi;
}
